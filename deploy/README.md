...
曼波音色 TTS 从零跑通的完整记录与复现步骤（时间线见 `docs/progress.md`）。
...
## 十六、⚠️ WSLg 音频桥：不能同时做采集和播放

**症状**：唤醒词命中了、HA 管线跑完了（监听中→处理中→回应中→空闲）、STT 也识别对了，
**但人什么都听不到**。表现像「喊了没反应」。

**真相**：音频出了 WSL 但在 WSLg 那座桥上被拖垮。8 秒音频、5 次取中位：

| 播放路径 | 语音栈停止 | 语音栈运行 |
|---|---|---|
| `ffmpeg → RDPSink` | **4 秒**（没播完就退出，砍掉后半段） | **34 秒** |
| `paplay → RDPSink` | 8 秒（正确） | **20 秒** |
| `scripts/snd_windows.sh`（Windows 原生） | — | **8 秒** ✓ |

**只要麦克风在采集，WSLg 的播放就慢 2.5~4 倍，换播放工具救不了。**
采集方向单独走是好的（STT 一直识别正常），坏的只有采集与播放**并发**这个组合。

顺带两个独立发现：
- `ffmpeg -f pulse` 播放时**不等播完就退出**（8 秒音频 4 秒返回），上层一回收进程尾巴就没了
- 两个并发播放流在 RDPSink 上会互相严重拖累（8 秒音频要 22~36 秒），
  所以「挂个常驻静音流保活」是**反效果**

**当前对策**：`scripts/snd_windows.sh` —— 把 PCM 写成 wav 丢进 Windows 临时目录，
用 PowerShell 的 `Media.SoundPlayer` 原生播放，绕开 WSLg。

⚠️ **它牺牲了流式**：要等音频收完才开始播。这跟「唤醒词 + 流式快速 TTS」的核心目标
冲突，所以是**权宜之计不是终局**。真正的解法方向（未做）：在 Windows 侧跑一个
接 TCP 流、边收边用 WASAPI 播的小服务，把流式恢复回来。

### ⚠️ 排查方法论：这次绕了很多弯路

在这个问题上连续提了五个错误假设（ffmpeg 双重重采样 / 麦克风饿死播放 /
`module-suspend-on-idle` / WSLg 通道断了 / ffmpeg 比 paplay 差），全被后续数据推翻。

共同错因：**拿单次「播放命令耗时」当指标**。那个数字把三段混在一起 ——
流启动延迟、实际播放时长、进程退出滞留 —— 同一条命令不同时刻能测出 7 秒和 34 秒。
噪声被当成了信号。

真正有效的只有两招：
1. **录 sink 监听端**（`-f pulse -i RDPSink.monitor`）＋ `silencedetect` 定位声音
   在时间轴上的位置，才看清「4 秒启动 + 正常播放 + 10 秒滞留」的三段结构
2. **人耳**：区分「完全没声音」和「有声音但断成两段」—— 这一句直接砍掉一半排查空间

**教训**：这个环境的音频测量噪声比信号大，任何单次测量都不可信，必须 N 次取中位。

### 遗留

- **KWS 服务空转烧 CPU**：实时率只有 0.015（比实时快 65 倍），却常年占 **92% 单核**，
  多做六十倍无用功。`scripts/wyoming_kws.py` 的循环写法有问题，待查。
  （已验证**不是**音频问题的原因：单核满载对播放无影响。）
- **真人语音召回 3/4**：合成样本 100%，真人偶尔要喊两次。
- 唤醒提示音延迟：走 Windows 原生后正常，但整体仍比理想慢。
